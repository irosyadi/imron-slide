...
**Solution:**
1.  Add $2 \times$ (first equation) to (second equation) to eliminate $x$:
...
**Solution:**
1.  Multiply the first equation by -3 and add it to the second equation:
...
**Solution:**
1.  Multiply the first equation by -4 and add it to the second equation:
...
To describe the infinite solutions, we use **parametric equations**.
Let $y = t$ (where $t$ is any real number). Then $4x - 2t = 1 \implies 4x = 1 + 2t \implies x = \frac{1}{4} + \frac{1}{2}t$.
...
## Example 5: A Linear System with Infinitely Many Solutions (3D)
