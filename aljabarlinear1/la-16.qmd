...
1.  **Existence:** Verify that $\mathbf{x} = A^{-1}\mathbf{b}$ is indeed a solution: $A(A^{-1}\mathbf{b}) = (AA^{-1})\mathbf{b} = I\mathbf{b} = \mathbf{b}$.
2.  **Uniqueness:** Assume $\mathbf{x}_0$ is any solution, so $A\mathbf{x}_0 = \mathbf{b}$. Multiply by $A^{-1}$ from the left: $A^{-1}(A\mathbf{x}_0) = A^{-1}\mathbf{b} \implies (A^{-1}A)\mathbf{x}_0 = A^{-1}\mathbf{b} \implies I\mathbf{x}_0 = A^{-1}\mathbf{b} \implies \mathbf{x}_0 = A^{-1}\mathbf{b}$.
...
$$
Solution for (a): $x_1 = 1, x_2 = 0, x_3 = 1$.
Solution for (b): $x_1 = 2, x_2 = 1, x_3 = -1$.
